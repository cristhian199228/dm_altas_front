<template>
    <v-container>
        <v-dialog v-model="dialog" persistent max-width="600px">
            <v-card>
                <v-card-title>SUBIR EVIDENCIAS MEDICAMENTO</v-card-title>
                <v-divider></v-divider>
                <v-card-text>
                    <br>
...
export default {
    ...
    computed: {
        dialog() {
            return this.$store.state.dialogEvidenciasMedicamento
        },
        photo() {
    ...
    methods: {
        ...
        close() {
            this.$store.commit('SET_DIALOG_EVIDENCIAS_MEDICAMENTO', false)
        },
        upload() {
            if (this.photo) {
